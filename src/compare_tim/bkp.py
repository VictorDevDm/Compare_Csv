import logging
import pandas as pd
from src.compare_vivo_GVI.database_utils import connect_to_db
from src.compare_vivo_GVI.config import gov_naturezas, gov_exceptions_cnpjs
...
# BACKUP DA FUNÇÃO CLASSFICANDO APENAS COM NATUREZA JURIDICA
# def add_company_details(df):
#     """
#     Adiciona detalhes da empresa ao DataFrame com base nos CNPJs.
#
#     Args:
#     - df (pd.DataFrame): O DataFrame contendo os dados a serem atualizados.
#
#     Returns:
#     - pd.DataFrame: O DataFrame atualizado com detalhes da empresa adicionados.
#     """
#     # Limpa e ajusta os CNPJs (CUST_ID_TEXT)
#     df['NR_DOCUMENTO'] = df['NR_DOCUMENTO'].apply(lambda x: str(x).strip().zfill(14) if pd.notnull(x) else '')
#
#     # Filtra empresas com CNPJ válido
#     df_com_cnpj = df[df['NR_DOCUMENTO'] != '']
#     cnpjs = df_com_cnpj['NR_DOCUMENTO'].unique().tolist()
#
#     # Busca detalhes das empresas
#     company_details = fetch_company_details(cnpjs)
#
#     # Atualiza o DataFrame com os detalhes da empresa
#     for col in ['natureza_juridica']:
#         df[col] = df['NR_DOCUMENTO'].map(lambda cnpj: company_details.get(cnpj, {}).get(col, ''))
#
#     # Classifica a empresa como PRIVADO ou GOVERNO com base na natureza jurídica
#     df['tipo_empresa'] = df['natureza_juridica'].apply(
#         lambda x: 'GOVERNO' if pd.notnull(x) and str(x).isdigit() and int(x) in gov_naturezas else
#                   'GOVERNO' if x['NR_DOCUMENTO'] in gov_exceptions_cnpjs
#                    else 'PRIVADO'
#     )
#
#     return df
...
def add_company_details(df):
    ...
    # Busca detalhes das empresas
    company_details = fetch_company_details(cnpjs)  # Presume que retorna um dict {cnpj: {'natureza_juridica': valor, ...}}

    # Atualiza o DataFrame com os detalhes da empresa
    df['natureza_juridica'] = df['NR_DOCUMENTO'].map(lambda cnpj: company_details.get(cnpj, {}).get('natureza_juridica', None))
    ...
    mask_cnpj = df['NR_DOCUMENTO'].isin(gov_exceptions_cnpjs)
...
if __name__ == "__main__":
    input_csv = '/home/victor-sims/Desktop/Compare_CSV/data/vivo-GVI/202411_Geral_Repasse_VGI.csv'
    output_csv = '/home/victor-sims/Desktop/Compare_CSV/data/vivo-GVI/202411_Geral_Repasse_VGI_Classificado.csv'
